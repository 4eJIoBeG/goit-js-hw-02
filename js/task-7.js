...
const isLoginValid = function (login) {
  let validFlag = false;
  login.length >= 4 && login.length <= 16 ? (validFlag = true) : validFlag;
  return validFlag;
};

const isLoginUnique = function (allLogins, login) {
  let validFlag = false;
  allLogins.includes(login) ? (validFlag = true) : validFlag;
  return validFlag;
};
...
console.log(addLogin(logins, 'alabama')); // 'Логин успешно добавлен!'
console.log(addLogin(logins, 'Ajax')); // 'Такой логин уже используется!'
console.log(addLogin(logins, 'jq')); // 'Ошибка! Логин должен быть от 4 до 16 символов'
console.log(addLogin(logins, 'Ajaxx')); // 'Логин успешно добавлен!'
